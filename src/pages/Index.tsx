...
const Index = ({ aiSearchEnabled, onToggleAISearch }: IndexProps) => {
  ...
  const [metallicImageData, setMetallicImageData] = useState<ImageData | null>(
    null
  );
    ...
    try {
      ...
      if (aiSearchEnabled) {
        ...
        const response = await fetch(
          `${API_ENDPOINTS.suggestions}?term=${encodeURIComponent(term)}`
        );
      ...
      const mockSuggestions = {
        movie: [
          {
            id: "1",
            label: "Wonder Woman (2017)",
            url: "/movies/38910-wonder-woman",
          },
          {
            id: "2",
            label: "Wonder Woman 1984 (2020)",
            url: "/movies/464052-wonder-woman-1984",
          },
        ],
        tv: [
          {
            id: "3",
            label: "Wonder Years (1988)",
            url: "/tv/1695-the-wonder-years",
            serial: "1",
          },
        ],
      };
      ...
      if (term.toLowerCase().includes("wonder")) {
        ...
        const hasResults =
          (mockSuggestions.movie && mockSuggestions.movie.length > 0) ||
          (mockSuggestions.tv && mockSuggestions.tv.length > 0);
        // Only show suggestions if we should show dropdown
        if (shouldShowDropdown) {
          setShowSuggestions(hasResults);
        }
        toast({
          title: "Demo Mode",
          description: `Showing sample results due to ${
            aiSearchEnabled ? "AI API" : "TMDB API"
          } limitations`,
          variant: "default",
        });
      }
  ...
  const fetchMovieRecommendations = async (
    ...
    try {
      updateHasError(false);
      const response = await fetch(
        `${API_ENDPOINTS.recommendations}?url=${encodeURIComponent(url)}`
      );
      ...
      if (response.ok) {
        ...
        if (contentType && contentType.includes("application/json")) {
          ...
          const movies: MovieData[] = Array.from(movieElements)
            .map((element, index) => {
              ...
              const container =
                element.closest(".column") || element.parentElement;
              const tvShowLabel = container?.querySelector(".label-default");
              const isTvShow =
                tvShowLabel?.textContent?.includes("TV show") || false;

              return {
                id: dataId,
                title: imgAlt.replace(/\s*\(\d{4}\)/, ""),
                poster: imgSrc.startsWith("/")
                  ? `https://bestsimilar.com${imgSrc}`
                  : imgSrc,
                year,
  ...
  const fetchTrendingMovies = async () => {
    ...
    try {
      ...
      toast({
        title: "Demo Mode",
        description:
          "Showing sample trending content due to TMDB API limitations",
        variant: "default",
      });
    } finally {
      setIsLoading(false);
        ...
        if (tmdbId) {
          ...
          if (!isNaN(numericId) && numericId > 0) {
            ...
            toast({
              title: "Not Found",
              description:
                "Could not find detailed information for this title.",
              variant: "destructive",
            });
          }
        }
  ...
  const handleTrendingMovieClick = useCallback(
    async (movie: any) => {
      try {
        // Persist current state before navigating
        persistState();
    ...
    <div className="min-h-screen bg-background text-foreground touch-manipulation relative">
        ...
        <div className="max-w-4xl mx-auto px-3 android-sm:px-4 py-4 android-sm:py-6">
          <div className="text-center mb-6 android-sm:mb-8">
            ...
            <p className="text-sm android-sm:text-base text-muted-foreground px-2">
              <span className="hidden android-md:inline">
                In this app you can{" "}
              </span>
                    ...
                    {suggestions.movie && suggestions.movie.length > 0 && (
                      <div className="mb-2">
                        ...
                        {suggestions.movie.map((item, index) => (
                          ...
                          >
                            <div className="text-sm android-sm:text-base text-foreground font-medium">
                              {item.label}
                            </div>
                          </div>
                        ))}
                    ...
                    {suggestions.tv && suggestions.tv.length > 0 && (
                      <div className="mb-2">
                        ...
                        {suggestions.tv.map((item, index) => {
                          const globalIndex =
                            (suggestions.movie?.length || 0) + index;
                          return (
                            <div
                              key={`tv-${item.id}`}
                              ref={(el) =>
          ...
          {hasError && (
            <div className="mt-2 p-2 android-sm:p-3 bg-yellow-50 border border-yellow-200 rounded-lg mx-1">
              <div className="flex items-center gap-2 text-yellow-800">
                ...
                <span className="text-xs android-sm:text-sm">
      ...
      <div className="max-w-7xl mx-auto px-3 android-sm:px-4 py-6 android-sm:py-8">
        {isLoading ? (
          ...
          <>
            {(() => {
              const moviesList = selectedMovies.filter(
                (item) => item.type === "movie"
              );
              const tvShowsList = selectedMovies.filter(
                (item) => item.type === "tv"
              );
              return (
                <>
                  ...
                  <div className="flex justify-center mb-6 android-sm:mb-8">
                    <div className="bg-muted rounded-xl p-1 flex w-full max-w-md android-sm:w-auto">
                      ...
                      >
                        ...
                        <span className="text-sm android-sm:text-base">
                      ...
                      {moviesList.length > 0 ? (
                        ...
                        <div className="text-center py-20">
                          ...
                          <p className="text-muted-foreground">
                            No movie recommendations available for this
                            selection.
                          </p>
                        </div>
                      )}
                    ...
                    >
                      {tvShowsList.length > 0 ? (
                        ...
                        <div className="text-center py-20">
                          ...
                          <p className="text-muted-foreground">
                            No TV show recommendations available for this
                            selection.
                          </p>
                        </div>
                      )}
